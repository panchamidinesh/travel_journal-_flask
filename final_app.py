...
from flask import Flask, render_template, request, redirect, url_for  #Used for rendering HTML templates.,request: Provides access to incoming request data.redirect: Used for URL redirection

import sqlite3

app = Flask(__name__)  #line creates a Flask application instance named app.
...
@app.route('/')  #decorator that associates the following function with the root URL (“/”) of the Flask application.
def home():
    with sqlite3.connect('travel_journal.db') as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM travel_entries")  #Fetches the results into the travel_entries variable.
    ...
    return render_template('final_app.html', travel_entries=travel_entries) #The template is rendered with the travel_entries data passed as a parameter.
#The template will display the travel entries on the web page.

@app.route('/add', methods=['POST'])  #decorator that associates the following function with the “/add” URL path and specifies that it should handle HTTP POST requests.
def add_entry():
...
@app.route('/edit/<int:entry_id>', methods=['POST'])  #The <int:entry_id> part indicates that the entry_id is expected to be an integer in the URL.v
